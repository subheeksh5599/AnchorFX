...
export async function GET(request: Request) {
  ...
  for (const e of escrows) {
    ...
    if (e.status === "Settled") {
      prev.completed++;
      if (e.settledAt > 0 && e.createdAt > 0)
        prev.settlementLedgers.push(e.settledAt - e.createdAt);
    }
    if (e.status === "Refunded") prev.refunded++;
    anchors.set(key, prev);
  }

  const results: AnchorRep[] = Array.from(anchors.entries()).map(
    ([addr, stats]) => ({
      ...
      avgSettlementLedgers: stats.settlementLedgers.length
        ? Math.round(
            stats.settlementLedgers.reduce((a, b) => a + b, 0) /
              stats.settlementLedgers.length
          )
        : 0,
      successRate:
